...
int thermodynamics_at_z(
  ...
  /**double H_twin_mass;
  H_twin_mass =pba->m_e_dark * _GeV_over_kg_ + pba->m_p_dark * _GeV_over_kg_ - 0.5 * pba->m_e_dark * _GeV_over_kg_ * pba->alpha_dark * pba->alpha_dark;**/
...
int thermodynamics_init(
  ...
  if (pba->has_twin == _TRUE_) {
        ...
        printf("Is the segfault here somehow?");
  }
  /** END TWIN SECTOR */
  ...
  printf("fHe_twin is %g",pth->fHe_twin);
  printf("Checkpoint");
  /* END TWIN SECTOR */
  ...
  /** START #TWIN SECTOR - Written by Jared Barron - these twin variables need to be defined.*/
...
int thermodynamics_workspace_init(
  ...
  ptw->z_H_twin_boltzmann_trigger = 0.1 * ptw->const_Tion_H_twin/ptw->Tnow_twin - 1.; /* Multiplier 0.05 is arbitrary - onset of recombination at 0.02 for SM is our guide here. This should be the redshift where the dark sector temperature is 0.05* the binding energy of hydrogen */
  printf("Trigger twin recombination with Boltzmann at z = %g",ptw->z_H_twin_boltzmann_trigger);
  ptw->z_H_twin_saha_trigger = 0.2 * ptw->const_Tion_H_twin/ptw->Tnow_twin - 1.;
  printf("Start evolving twin H with saha at %g",ptw->z_H_twin_saha_trigger);
  //ptw->z_He1_twin_trigger = 0.1 * ptw->const_Tion_HeI_twin/ptw->Tnow_twin - 1.;
  //ptw->z_He2_twin_trigger = 0.1 * ptw->const_Tion_HeII_twin/ptw->Tnow_twin - 1.;
  ...
  /* BEGIN #TWIN SECTOR */
  /*ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_He1_twin] = 0.95 * 1e6 // First twin He-recombination (HeIII)
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_He1f_twin] = 0.9 * 1e6 // in between 1st and 2nd twin He recombination 
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_He2_twin] = 0.8 * 1e6 //  beginning of 2nd twin He-recombination*/
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_H_twin] = ptw->z_H_twin_saha_trigger;  // beginning early twin H-recombination 
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_Hf_twin] = ptw->z_H_twin_botlzmann_trigger;  // beginning full twin H-recombination 
  /* END TWIN SECTOR */
  ...
  /* BEGIN #TWIN SECTOR */
  /**ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_He1_twin] = 50;
  ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_He1f_twin] = 50;
  ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_He2_twin] = 50;*/
  ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_H_twin] = 100; //50 seems to be ok for SM stuff. Check back again later. 
  ptw->ptdw->ap_z_limits_delta[ptw->ptdw->index_ap_Hf_twin] = 100; //50 seems to be ok for SM stuff. 
  ...
  /* BEGIN #TWIN SECTOR DO I Need this? CHECK */
    ...
    ptw->ptdw->phyrec_twin->thermohyrec_verbose = pth->hyrec_verbose;
    class_call(thermodynamics_hyrec_init(ppr,pba,pth,ptw->SIunit_nH0,ptw->Tnow_twin,ptw->fHe_twin, ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_brec],ptw->ptdw->phyrec_twin),
...
int thermodynamics_vector_init(
  ...
  /* BEGIN #TWIN SECTOR */
  /* These are in backwards time order */
  //if (z < ptw->z_H_twin_boltzmann_trigger) {
    //class_define_index(ptv->index_ti_x_He_twin,_TRUE_,index_ti,1); /*- this is only needed if twin helium is being evolved with a boltzmann equation*/
    //class_define_index(ptv->index_ti_x_H_twin,_TRUE_,index_ti,1);   
  }
  
  /*else if (z < ptw->z_H_twin_saha_trigger) {
    class_define_index(ptv->index_ti_x_He_twin,_TRUE_,index_ti,1);
  }
  else if (z < ptw->z_He1_twin_trigger) {
  }

  else if (z < ptw->z_He2_twin_trigger) {
  }
  ...
  printf("The number of things that need to be integrated is %d",index_ti);
  ...
  if (ptdw->ap_current == ptdw->index_ap_brec) {
    ...
    /* BEGIN #TWIN SECTOR */
    ptdw->Tmat_twin = ptw->Tnow_twin * (1.+z);//without twin helium. 
    /* END TWIN SECTOR */
    ...
    /* BEGIN #TWIN SECTOR */
    ptdw->ptv->y[ptdw->ptv->index_ti_D_Tmat_twin] = 0.;//without twin helium.     
    /* END TWIN SECTOR */
    ...
    ptv->y[ptv->index_ti_D_Tmat_twin] = ptdw->ptv->y[ptdw->ptv->index_ti_D_Tmat_twin];
    ptv->y[ptv->index_ti_x_H_twin] = ptdw->x_H_twin;//ptdw->ptv->y[ptdw->ptv->index_ti_x_H_twin];    
  ...
  /* - in this scheme we start to evolve Helium and thus need to set its initial condition via the analytic function */
    ...
    /* BEGIN #TWIN SECTOR */   
    /*if (z > ptw->z_H_twin_saha_trigger) {
    ...
    else if (z > ptw->z_H_twin_boltzmann_trigger) {
  ...
  /* - in all other approximations, which are SM helium, we only evolve Tmat and twin quantities, and set its initial conditions from the previous scheme.  */
  else{
...
int thermodynamics_derivs(
  ...
  case hyrec:
    ...
    /* BEGIN #TWIN SECTOR: WILL THIS JUST WORK???? CHECK. */
    /* Hydrogen equations */
    if (ptdw->require_H_twin == _TRUE_) {
      class_call(hyrec_dx_H_dz(pth,ptw->ptdw->phyrec_twin,x_H_twin,x_He_twin,x_twin,nH_twin,z,Hz,Tmat_twin,Trad_twin,pba->alpha_dark/0.00729735,_m_e_twin/_m_e_,&(dy[ptv->index_ti_x_H_twin])),
                 ptw->ptdw->phyrec_twin->error_message,
                 error_message);
    }
  ...
  **/
  ...
  if ( ap_current == ptdw->index_ap_brec) {
    ...
    //printf("The index of the matter temperature is %g",ptdw->ptv->index_ti_D_Tmat);
    dy[ptdw->ptv->index_ti_D_Tmat] = eps * depsdlna / (1.+z);
    printf("Derivative of matter temperature is %g \n",dy[ptdw->ptv->index_ti_D_Tmat]);
  }
  ...
  if ( z > ptw->z_H_twin_saha_trigger) {
    /* Early time steady state equation */
    printf("Computing twin baryon temperature with steady state at z=%f \n",z);
    dHdlna = (1.+z)*pvecback[pba->index_bg_H_prime]/pvecback[pba->index_bg_H] * _c_ / _Mpc_over_m_;
    eps_twin =  Trad_twin * Hz / rate_gamma_b_twin;
    depsdlna_twin = dHdlna/Hz + 3.;
    /* Recfast v 1.5: add here a smoothing term as suggested by Adam Moss */
    //printf("The index of the twin matter temperature is %g",ptdw->ptv->index_ti_D_Tmat_twin);
    dy[ptdw->ptv->index_ti_D_Tmat_twin] = eps_twin * depsdlna_twin / (1.+z);
    printf("Derivative of twin matter temperature is %g \n",dy[ptdw->ptv->index_ti_D_Tmat]);

  }

  else {
    /* Full equations at later times */
    //printf("2The index of the twin matter temperature is %g",ptdw->ptv->index_ti_D_Tmat_twin);
    printf("Computing twin baryon temperature with bolztmann at z=%f \n",z);
...
int thermodynamics_sources(
  ...
  pth->thermodynamics_table[(pth->tt_size-index_z-1)*pth->th_size+pth->index_th_dkappa_twin] = (1.+z) * (1.+z) * pth->Nnow_twin * x_twin * _sigma_twin * _Mpc_over_m_;
...
int thermodynamics_ionization_fractions(
  ...
  /* BEGIN #TWIN SECTOR */
  double x_H_twin,x_He_twin,xHeII_twin,x_twin=0.,Tmat_twin,rhs_twin,sqrt_val_twin,rescale_rhs_twin = 1.;
  /* END TWIN SECTOR */
  ...
  if (z < ptw->z_H_twin_boltzmann_trigger) {
    ...
    x_twin = x_H_twin; //+ ptw->fHe_twin * x_He_twin; uncomment if you want to use twin helium. 
    ...
    /* Assuming Saha equilibrium for twin HII->HI. Includes xHeII corrections from incomplete recombination of HeII --> HeI (non-zero x_HeII) */
    //This has twin helium. For now we're going to omit twin helium until we get it working with twin hydrogen. 
    ...
    x_H_twin = 2./(1 + sqrt(1 + 4./rhs_twin));
    x_twin = x_H_twin;
  ...
  //else if (z < ptw->z_He1_twin_trigger) {
    /* Assuming Saha equilibrium for twin HeII --> HeI with HII fully ionized */
