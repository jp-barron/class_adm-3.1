...
int hyrec_dx_H_dz(struct thermodynamics* pth, struct thermohyrec* phy, double x_H, double x_He, double xe, double nH, double z, double Hz, double Tmat, double Trad, double alphaR, double meR, double *dx_H_dz) {
  ...
  /* BEGIN #TWIN SECTOR */
  /* TEMP TWIN FLAG */
  else if (phy->data->cosmo->fsR != 1. && pth->has_twin == _TRUE_){ model = PEEBLES;}//EMLA2s2p;}//Don't use correction function for radiative transfer effects in dark sector, because we can't rely on the linear approximation used to compute it. Rather neglect the effects entirely. 
