...
int hyrec_dx_H_dz(struct thermodynamics* pth, struct thermohyrec* phy, double x_H, double x_He, double xe, double nH, double z, double Hz, double Tmat, double Trad, double alphaR, double meR, double *dx_H_dz) {
  ...
  if (Trad_phys > TR_MAX || Trad_phys <= TR_MIN || Tmat/Trad <= T_RATIO_MIN) { model = PEEBLES; }//TEMP MARCH 28printf("Using peebles\n");
  /* BEGIN #TWIN SECTOR */
  /* TEMP TWIN FLAG */
  else if (phy->data->cosmo->fsR != 1. && pth->has_twin == _TRUE_){ model = EMLA2s2p;}//TEMP MARCH 28 printf("Using EMLA\n");//Don't use correction function for radiative transfer effects in dark sector, because we can't rely on the linear approximation used to compute it. Rather neglect the effects entirely. 
