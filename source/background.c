...
int background_functions(
  ...
  if (pba->has_twin == _TRUE_) {
    ...
    /* GREG CHECK Note Jan 15 2025: Where does this expression come from? Baumann? How do we add protons to it? */
  ...
  if (return_format == long_info) {
    ...
    /*  */
    /** START #TWIN SECTOR */
    /** Added by Jared Barron */
    if (pba->has_twin == _TRUE_) {
      pvecback[pba->index_bg_rs_twin] = pvecback_B[pba->index_bi_rs_twin];
    }
    /** END TWIN SECTOR */
  }
...
int background_checks(
  ...
  if (pba->background_verbose > 0) {
    ...
    if (pba->has_idr == _TRUE_ && pba->r_all_twin == 0.) {
      N_dark = pba->Omega0_idr/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
      printf(" -> dark radiation Delta Neff %e\n",N_dark);
    }
  }
  /** START #TWIN SECTOR : Moved by Jared Barron*/
  /* TWIN Print the twin sector Neff and other twin quantities */
  if (pba->r_all_twin != 0.) {
    N_twin = pba->Omega0_g_twin/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
    if (pba->background_verbose > 0) {
...
int background_derivs(
  ...
  if (pba->has_twin == _TRUE_) {
  dy[pba->index_bi_rs_twin] = 1./a/H/sqrt(3.*(1.+3.*pvecback[pba->index_bg_rho_b_twin]/4./pvecback[pba->index_bg_rho_g_twin]))*sqrt(1.-pba->K*y[pba->index_bi_rs_twin]*y[pba->index_bi_rs_twin]);
  }
  /** END TWIN SECTOR */
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    /* TWIN - index for rho_g_twin (mirror photon density) */
    if(pba->has_twin){
      class_print_species("Twin photon relics",g_twin);
      class_print_species("Twin neutrino relics",ur_twin);
