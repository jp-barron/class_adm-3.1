...
int thermodynamics_workspace_init(
  ...
  ptw->z_H_twin_boltzmann_trigger = 0.05 * ptw->const_Tion_H_twin/ptw->Tnow_twin - 1.; /* Multiplier 0.05 is arbitrary - onset of recombination at 0.02 for SM is our guide here. This should be the redshift where the dark sector temperature is 0.05* the binding energy of hydrogen */
  printf("Trigger twin recombination with Boltzmann at z = %g",ptw->z_H_twin_boltzmann_trigger);
  ptw->z_H_twin_saha_trigger = 0.1 * ptw->const_Tion_H_twin/ptw->Tnow_twin - 1.;
  ...
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_H_twin] = 0.7 * 1e6 // beginning early twin H-recombination 
  ptw->ptdw->ap_z_limits[ptw->ptdw->index_ap_Hf_twin] = 0.6 * 1e6 // beginning full twin H-recombination */
  /* END TWIN SECTOR */
...
int thermodynamics_vector_init(
  ...
  printf("The number of things that need to be integrated is %g",index_ti);
  ...
  if (ptdw->ap_current == ptdw->index_ap_brec) {
    ...
    ptdw->ptv->y[ptdw->ptv->index_ti_D_Tmat] = 0.;
      ...
      if (z > ptw->z_He2_twin_trigger) {
        ...
        ptdw->require_He_twin = _FALSE_;  /*Set to true if twin helium recombination with boltzmann is being done. */
        ptdw->require_H_twin = _FALSE_;
      }
      else if (z < ptw->z_H_twin_boltzmann_trigger) {
        ptdw->Tmat_twin = ptdw->ptv->y[ptdw->ptv->index_ti_D_Tmat_twin] + ptw->Tnow_twin*(1.+z);
...
int thermodynamics_derivs(
  ...
  //printf("matter temperature is %g",Tmat);
